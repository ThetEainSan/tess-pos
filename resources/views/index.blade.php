                    ...
                    <div class="col-md-3 relative right-padding-default clear-padding" id="slide-bar-category">
                        ...
                        <div class="col-sm-12 col-sm-12 col-xs-12 sider-bar-category border bottom-margin-default">
                            ...
                            <ul class="clear-margin list-siderbar">
                                <li>Foods</li>
                                @php
                                    $foodTypes = App\Models\Inventory::where('FOD', 'foods')
                                        ->select('type')
                                        ->distinct()
                                        ->get();
                                    $drinkTypes = App\Models\Inventory::where('FOD', 'drinks')
                                        ->select('type')
                                        ->distinct()
                                        ->get();
                                @endphp
                                <ul style="padding-left: 10px">
                                    @foreach ($foodTypes as $foodType)
                                        <li><a
                                                href="{{ route('typeSearch', ['type' => $foodType->type]) }}">{{ $foodType->type }}</a>
                                        </li>
                                    @endforeach
                                </ul>
                                <li>Drinks</li>
                                <ul style="padding-left: 10px">
                                    @foreach ($drinkTypes as $drinkType)
                                        <li><a
                                                href="{{ route('typeSearch', ['type' => $drinkType->type]) }}">{{ $drinkType->type }}</a>
                                        </li>
                                    @endforeach
                                </ul>
                            </ul>
                                ...
                                <div
                                    ...
                                    <div class="image-product relative overfollow-hidden">
                                        ...
                                        <ul class="option-product animate-default">
                                            <li class=""><a
                                                    href="{{ route('addToCart', ['id' => $food->id]) }}">Order</a></li>
                                        </ul>
                                    </div>
                                    <h3 class="title-product clearfix full-width title-hover-black">{{ $food->name }}
                                    </h3>
                                    <p class="clearfix price-product" style="color: rgb(75, 75, 112)">{{ $food->price }}
                                        MMK</p>
                                    <div class="clearfix ranking-product-category ranking-color">
                                        <div>Stocks Left : {{ $food->quantity }}</div>
                                    </div>
                                </div>
                            @endforeach
                            <div class="bar-category bottom-margin-default border no-border-r no-border-l no-border-t">
                                <div class="row">
                                    <div class="col-md-5 col-sm-5 col-xs-4">
                                        <h3 class="title-category-page clear-margin">Drinks</h3>
                                    </div>
                                </div>
                            </div>
                            @foreach ($drinks as $drink)
                                <div
                                    ...
                                    <div class="image-product relative overfollow-hidden">
                                        <div class="center-vertical-image">
                                            <img class="img-responsive"
                                                src="{{ env('ADMINSITE_URL') . 'img/drink/' . $drink->image }}"
                                                alt="Product" />
                                        </div>
                                        <a href="#"></a>
                                        <ul class="option-product animate-default">
                                            <li class=""><a
                                                    href="{{ route('addToCart', ['id' => $drink->id]) }}">Order</a></li>
                                        </ul>
                                    </div>
                                    <h3 class="title-product clearfix full-width title-hover-black">{{ $drink->name }}
                                    </h3>
                                    <p class="clearfix price-product" style="color: rgb(75, 75, 112)">{{ $drink->price }}
                                        MMK</p>
                                    <div class="clearfix ranking-product-category ranking-color">
                                        <div>Stocks Left : {{ $drink->quantity }}</div>
                                    </div>
                                </div>
                            @endforeach
                        </div>
